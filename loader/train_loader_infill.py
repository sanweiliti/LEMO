    ...
    def create_body_repr(self, with_hand=False, smplx_model_path=None):
        ...
        for i in tqdm(range(self.n_samples)):
            ...
            if self.mode in ['local_joints', 'local_joints_4chan']:
                ...
                is_contact = torch.abs(foot_joints_vel) < 0.22  # velocity threshold
            ...
            if self.mode in ['local_markers', 'local_markers_4chan']:
                ...
                foot_markers = torch.cat([markers[:, 16:17], markers[:, 47:48], markers[:, 30:31], markers[:, 60:61]],
                                         dim=-2)  # [T, 4, 3]
            ...
            cur_body = cur_body.detach().cpu().numpy()  # numpy, [T, n, 3], in (x,y,z)
            ...
            cur_body = np.concatenate([reference[:, np.newaxis], cur_body], axis=1)  # [T, 1+n, 3]

            """ Get Root Velocity in floor plane """
            velocity = (cur_body[1:, 0:1] - cur_body[0:-1, 0:1]).copy()  # [T-1, 3] ([:, 1]==0)

            """ To local coordinates """
            cur_body[:, :, 0] = cur_body[:, :, 0] - cur_body[:, 0:1, 0]  # [T, 1+n, 3]
            ...
            cur_body = rotation * cur_body  # [T, 1+n, 3]
            ...
            cur_body = cur_body[0:-1, 1:, :]  # [T-1, n, 3]
            cur_body = cur_body.reshape(len(cur_body), -1)  # [T-1, n*3]
            ...
            if self.mode in ['local_joints', 'local_markers']:
                ...
                cur_body = np.concatenate([global_vel, cur_body, contact_lbls[0:-1]], axis=-1)  # [T-1, d=3+n*3+4]

            elif self.mode in ['local_joints_4chan', 'local_markers_4chan']:
                channel_local = np.concatenate([cur_body, contact_lbls[0:-1]], axis=-1)[np.newaxis, :, :]  # [1, T-1, d=n*3+4]
        ...
        if self.normalize:
            # prefix = 'prestats_contact_given_global_new'
